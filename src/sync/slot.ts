 ...
 * - The internal chain NEVER rejects. {@link CaptureSlot.trigger} is
 *   fire-and-forget and nothing awaits what it starts, so a rejection there
 *   becomes an unhandled rejection and, under Node's defaults, kills the host
 *   process. Both throw sites inside the drain — the capture itself and the
 *   host's failure callback — are contained.
 */
export class CaptureSlot {
  ...
  /**
   * Deliver a capture failure to host code without letting it escape.
   *
   * A throwing handler would reject {@link CaptureSlot.#drain}, and the drain
   * is started by the fire-and-forget {@link CaptureSlot.trigger}; nobody is
   * there to catch it. The handler's own bug is reported and dropped, and the
   * slot goes on to its next capture.
   */
  #notifyFailure(error: Error): void {
    try {
      this.#onFailure?.(this.#failures, error)
    } catch (handlerError: unknown) {
      console.error('TuddoFs capture failure handler threw', handlerError, { error })
    }
  }
