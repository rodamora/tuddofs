...
- **`packages/s3` had no publish gate.** It now has a `prepublishOnly` running build + typecheck + the conformance suite; verified by `npm publish --workspace @tuddo/s3 --dry-run` with MinIO up.
