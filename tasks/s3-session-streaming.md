...
Status: done (review fixes and observed acceptance evidence below)
...
Big media moves through a typed `session.mount(key)` handle without transiting server memory: `readStream`, `writeStream` (hash-on-the-fly, quarantine-then-promote), and `presign` for client-direct I/O.

## Scope

- `mount.readStream(path)` → `Readable` from `BlobStore.get` for CAS blobs; inline blobs wrap a buffer. Grant check identical to `readBytes`.
- `mount.writeStream(path, source)` → sha256 transform stream while uploading to a quarantine key; on end, server-side copy to `tuddo/<tenant>/<sha>` then the §4.5 tx (upload BEFORE tx, as always). Identity binds to bytes actually stored.
- `mount.presign(path)` → GET presigns for reads; PUT presigns pin `x-amz-checksum-sha256` as a signed header. Requires the store's optional `presignPut`/`presignGet`; absent → typed `StorageError`, never silent fallback.
...
- Tier-1 surface additions (§6.2) stay on the typed mount handle only: `readStream`/`writeStream`/`presign`, nothing else.
...
- RSS diagnostic — baseline 188,280,832 bytes; peak 306,757,632; growth 118,476,800; asserted ceiling 402,653,184.
...
- `TUDDOFS_DATABASE_URL=postgresql://tuddofs:tuddofs@127.0.0.1:55435/tuddofs_it npm run test:integration` — all 100 tests passed, including all 9 focused streaming regressions.
- `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run build`, and `npm test` all passed after rebasing onto the Tier-1 surface cut; the unit suite passed all 63 tests.
