...
**AMENDED at S1 — the straggler guard is WINDOWED at BOTH ends, and it survives resume.** Three clarifications the implementation forced, all load-bearing:

- The guard covers the window between a Phase-2 commit and the first scan that observes those bytes on disk. That scan RETIRES it. Held open for the session, the guard misreads every later return to the pre-write content — `git checkout`, a formatter, an undo — as a lost mirror write, re-materializes over the agent's file, commits nothing and fires no event: silent, permanent loss, armed for the rest of the session. The same rule applies to the absent-on-disk case: a path is protected from deletion only until a scan has confirmed its mirror write. Consecutive writes before that first confirming scan compare against the last CONFIRMED sha, not the intermediate one the mirror may never have held.
- **An EXEC closes the window too, and it is the bound that makes the guard safe.** The confirming scan alone is not enough: an exec that reverts a file in the SAME turn, before any scan has observed the write, puts the pre-write sha back on disk, which is bit-for-bit what a lost mirror write looks like. Content cannot separate them — a revert lands on a sha the path genuinely held — so the discriminator is provenance: only an exec can change the mirror behind the engine's back, and the engine knows exactly when one ran (`exec`, and `captureAfterExec` for hosts running their own). Divergence with no exec since the commit can ONLY be the lost mirror write; divergence after one is the exec's work and is CAPTURED, never re-materialized. Both halves are bounded: content divergence and absence alike. The residual case — a `writeFile` that resolves while dropping its bytes, followed by an exec — is captured as an exec change, moving the branch head back one version; that is the cheap failure, because it needs a lying target (a kill rejects the write and marks the path dirty; a crash goes through the resume rule below) and it lands as a `capture` commit with an `onCapture` event, leaving the superseded bytes in history. Guarding instead deletes the exec's output from disk AND from the branch with no commit and no event.
- The guard is process memory, and §7.5 line 1 ("tool write survives instant target kill") has to hold across a restart too. A warm re-acquire (phase 1 step 3) seeds the index from heads, which carry no straggler state, so a crash between commit and mirror write would leave reconcile committing the stale disk bytes back over the durable commit — or deleting a committed file the mirror never received. On a warm-acquired mount, therefore, divergence is checked against DURABLE HISTORY until the first authoritative scan retires the guard or the first exec of the resumed process claims the mirror, whichever comes first — the same two bounds, since a post-resume exec revert is the same ambiguity as the in-memory one. Within that window, a divergent already-committed path whose disk content is a sha the path held ANYWHERE in its lineage is a stale mirror and the durable HEAD wins: re-materialize. Only content from nowhere in the lineage is uncaptured exec work and is committed. Comparing against just the newest write's parent is not enough — two consecutive lost mirror writes leave the disk two versions back, and reconcile then commits the head away. For the absent-on-disk case the test is unchanged: if the newest commit is a Phase-2 `write` that created the path, the mirror write never landed and the path is re-materialized. A `capture` commit is never a candidate — its bytes were READ off the mirror, so they were on disk by construction. The lookup is one history query plus at most one diff per commit that touched the path, for divergent paths only, once per resume, and never touches the write path. Persisting the index into the target was considered and REFUSED: the target is untrusted input (§13 never-do list), and guard state read back from it would let a target direct the engine to overwrite files.
...
Ship `@tuddo/s3` (or equivalent) as a separate package implementing `BlobStore` against S3/MinIO/R2. Core keeps zero storage SDKs. A standalone product with a never-implemented SPI is a contract nobody has honored — the adapter is what proves it.
...
| S3  | Large blobs (§8): session streaming + presign issuance; sync capture path; `@tuddo/s3` reference adapter                                                                                                                     | 2 GB MinIO round-trip, flat RSS, both paths; presign contract tests                                                                                                                                                                                                                |
...
3. **Reference storage adapter name — CLOSED: `@tuddo/s3`, settled under npm org `tuddo`.** Separate package, zero dependency on core (it implements the structural SPI), and it carries the reusable `@tuddo/s3/conformance` kit so a third-party adapter can prove itself against the same suite.
4. **Sync engine placement — CLOSED: core, published through `tuddofs/internal`.** It imports no provider SDK and no target implementation, only the four-verb seam, so a separate sync package would split the R2/R3/R4 answer across two release cadences for zero decoupling. Placement in core is not the same question as tier: the engine, its targets, and `SyncTargetError` stay on the `tuddofs/internal` subpath rather than the Tier-1 storefront (§6.2), because Tier 1 is the surface every host sees and most hosts never run an engine. Promoting the engine to Tier 1 later is an additive, semver-minor change; taking it back is not, so the small surface is the reversible choice.
