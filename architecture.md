...
6. `touch -d @<scanStart> .tuddofs-stamp`. Files written during the scan re-appear next cycle; sha-diff makes re-capture a no-op.
...
## 12. Performance budgets (absolute; measured in S1/S2)
...
4. **Perf budgets are estimates until S1/S2 measure** — replace §12 numbers with measurements, keep them as regressions.
